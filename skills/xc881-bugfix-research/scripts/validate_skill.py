...
for ref in ['web-research-policy.md', 'bugfix-forensics-workflow.md', 'vulnerability-triage-policy.md', 'reproducer-verification-plan.md', 'bugfix-output-contract.md']:
...
if len(text.splitlines()) > 95: die("SKILL.md too long")
for ref in (root/"references").glob("*.md"):
    if len(ref.read_text(encoding="utf-8").splitlines()) > 45:
